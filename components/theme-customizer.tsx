...
export function ThemeCustomizer() {
  ...
  const applyThemeColors = useCallback(
    (config: ThemeConfig, isDark: boolean) => {
      ...
      .sidebar-section-label {
        color: hsl(${
          colors.menuSectionLabel || colors.mutedForeground
        }) !important;
      }
  ...
  // Apply config from localStorage and apply initial theme immediately
  useEffect(() => {
    ...
    // Apply theme immediately, even before mounted state
    const currentTheme = document.documentElement.classList.contains("dark")
      ? "dark"
      : "light";
    applyThemeColors(finalConfig, currentTheme === "dark");

    // Apply menu state immediately
    if (
      ...
      existingStyle.textContent =
        existingStyle.textContent
          ?.replace(
            ...
            `.sidebar-menu-text { color: hsl(${
              colors.menuText || colors.foreground
            }) !important; }`
          )
          .replace(
            /\.sidebar-menu-icon\s*{[^}]*}/g,
            `.sidebar-menu-icon { color: hsl(${
              colors.menuIcon || colors.foreground
            }) !important; }`
          )
          .replace(
            /\.sidebar-menu-item:hover \.sidebar-menu-text\s*{[^}]*}/g,
            `.sidebar-menu-item:hover .sidebar-menu-text { color: hsl(${
              colors.menuTextHover || colors.foreground
            }) !important; }`
          )
          .replace(
            /\.sidebar-menu-item:hover \.sidebar-menu-icon\s*{[^}]*}/g,
            `.sidebar-menu-item:hover .sidebar-menu-icon { color: hsl(${
              colors.menuIconHover || colors.foreground
            }) !important; }`
          )
          .replace(
            /\.sidebar-section-label\s*{[^}]*}/g,
            `.sidebar-section-label { color: hsl(${
              colors.menuSectionLabel || colors.mutedForeground
            }) !important; }`
          ) || "";
    }
  ...
  const handleSave = () => {
    ...
    // Immediately reapply theme to ensure consistency
    const isDark = theme === "dark";
    applyThemeColors(config, isDark);
  ...
  const handleReset = () => {
    ...
    // Immediately apply default theme
    const isDark = theme === "dark";
    applyThemeColors(defaultConfig, isDark);
    ...
    <>
      ...
      {isOpen && (
        <>
          ...
          {/* Panel */}
          <div
            className={`fixed ${panelPosition} top-0 h-full w-80 bg-white dark:bg-gray-900 border-${
              config.layout === "rtl" ? "r" : "l"
            } border-gray-200 dark:border-gray-700 z-[100] overflow-y-auto`}
          >
            <div className="p-4">
              {/* Header */}
